...
looker.plugins.visualizations.add({
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) { 
    ...
    for (i = 1; i <= 2; i++) {
      let iN = (i+1).toString()
      options['x_' + iN]      = {order: i*7+0, label: "x axis (" + iN + ")", default: queryResponse.fields.dimension_like[0].name }
      options['x_LB' + iN]    = {order: i*7+1, label: "x error lower bound (" + iN + ")", display_size: "half"}
      options['x_UB' + iN]    = {order: i*7+2, label: "x error upper bound (" + iN + ")", display_size: "half"}
      options['y' + iN]       = {order: i*7+3, label: "y axis (" + iN + ")", default: queryResponse.fields.measure_like[0].name }
      options['y_LB' + iN]    = {order: i*7+4, label: "y error lower bound (" + iN + ")", display_size: "half"}
      options['y_UB' + iN]    = {order: i*7+5, label: "y error upper bound (" + iN + ")", display_size: "half"}
      options['values' + iN]  = {order: i*7+6, label: "custom values (" + iN + ")", display_size: "half"}
      options['hover' + iN]   = {order: i*7+7, label: "custom hovertext (" + iN + ")", display_size: "half"}
    }
